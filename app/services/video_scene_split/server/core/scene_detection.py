...
class SceneDetector:
    def __init__(self, model_path="/app/server/models/transnetv2-weights/"):
    ...
    def _get_video_frames_generator(self, video_path, batch_size=32):
        """生成器函数，分批yield视频帧"""
    ...
    def process_video(self, video_path, output_dir, batch_size=32):
        ...
        print("正在处理视频帧...")
        for batch_frames in tqdm(
            self._get_video_frames_generator(video_path, batch_size),
            total=(total_frames + batch_size - 1) // batch_size,
        ):
            batch_frames_5d = np.expand_dims(batch_frames, axis=-1)  # 加入时间维度
            batch_frames_5d = np.repeat(
                batch_frames_5d, 3, axis=-1
            )  # 重复帧数据，确保 depth 是 3 的倍数
        ...
        for i in range(len(scene_transitions) - 1):
            ...
            output_path = os.path.join(
                output_dir, f"scene_{i:04d}_{start:06d}_{end:06d}.mp4"
            )
            out = cv2.VideoWriter(
...
def main():
    ...
    try:
        ...
        detector.process_video(args.video, args.output, args.batch_size)
        print("处理完成！")
